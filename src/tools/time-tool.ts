...
const TOOL_HANDLER = async (args: ToolArgs, extra: any) => {
  const { time_description, timezone: requestedTimeZone } = args;
  // null timezone = the organization's (resolveOrgTimeZone), not the server's.
  const timezone =
    requestedTimeZone ??
    (await resolveOrgTimeZone(extra?._meta?.requestModifiers, extra?.sessionId));
  ...
  const result = parseTimeDescription(time_description ?? undefined, timezone, extra);
