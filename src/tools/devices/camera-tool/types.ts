...
export const ExternalUpdateableFacetedUserConfigSchema = z.object({
  ...
  videoFacetSettings: z.record(
    z.object({
      ...
      img_brightness: z.number().nullable().describe("Ranges from [-255, 255]. The image is described as a range from [0, 255], and the brightness slider will ADD this value to whatever the original value is. Use this accordingly."),
      img_contrast: z.number().nullable().describe("Ranges from [0, 128]. In general, 64 works for most situations, but adjust accordingly."),
      img_saturation: z.number().nullable().describe("Ranges from [0, 255]. In general, 64 works for most situations, but adjust accordingly."),
      img_sharpness: z.number().nullable().describe("Ranges from [0, 11]. In general, 6 works for most situations, but adjust accordingly."),
  ...
  ).nullable().describe("This describes the kind of settings that you can manipulate for a camera. If the user asks to change settings, only change what is necessary and don't pass in any fields that you do not need."),
});

export type ExternalUpdateableFacetedUserConfig = z.infer<typeof ExternalUpdateableFacetedUserConfigSchema>;